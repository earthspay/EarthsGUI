    ...
    const factory = function (state, user, $state) {
        ...
        class StateManager {
            ...
            getSrefByState(state) {
                return EarthsApp.stateTree.getPath(state.id).join('.');
            }

            getStateTree(fromId = 'main') {
                const root = EarthsApp.stateTree.find(fromId);
            ...
            _currentStateLists() {
                ...
                const root = EarthsApp.stateTree.find('main');
                const rootPath = EarthsApp.stateTree.getPath(root.id).join('.');
                ...
                const stateData = EarthsApp.stateTree.find(parent);
                if (stateData && !stateData.get('abstract')) {
                    this.subStateList = stateData.getChildren().map((item) => {
                        const path = EarthsApp.stateTree.getPath(item.id).join('.');
